...
| Link | Sha |
| ---- | --- |
| [Zip File](https://github.com/square/bazel_maven_repository/archive/1.1.zip) | None |
| [Tarball](https://github.com/square/bazel_maven_repository/archive/1.1.tar.gz) | None |
...
```python
MAVEN_REPOSITORY_RULES_VERSION = "<current_version>"
MAVEN_REPOSITORY_RULES_SHA = "<current_sha>"
http_archive(
...
maven_repository_specification(
    ...
    artifacts = {
        ...
        "android.arch.lifecycle:livedata-core:1.1.1": { "sha256": "fedcba" },
    },
    repository_urls = [
...
Currently `.aar` and `.jar` artifacts are supported, as well as OSGI bundles.  The tool will
infer the packaging type from the `pom.xml` file.
...
Bazel has constraints on target names.  Some artifacts contain ".", for example
`javax.inject:javax.inject:1`. bazel_maven_repository will _mangle_ the `artifact_id` turning
`javax.inject` into `javax_inject`.  For instance:
...
`joda-time`, in this example, was not _mangled_, while `javax.inject` was.
...
There is a backward-compatible legacy mode which would also _mangle_ `joda_time`.  It can be
enabled by configuring `legacy_artifact_id_munge = True`. It exists to ease migration from the
older, more aggressive _mangle_ used in version 1.0 and older.
...
An artifact can be listed as "testonly" which will [cause bazel][testonly] to ensure it can only be used as a
dependency by a test, or by a library also marked with `testonly = True`
...
An artifact can be configured to trim its direct dependencies (and thereby its transitive closure)
by offering a list of group_id/artifact_id pairs.
...
maven_repository_specification(
    ...
    artifacts = {
        "com.google.truth:truth:0.42": {
            "insecure": True,
            "exclude": ["org.checkerframework:checker-qual"], # for demonstration only
        },
    }
)
```
...
> Note: You must specify the packaging if you specify the classifier, as in the example above.
...
| BMR Version    | 1.0        | 1.1    | HEAD   |
| -------------- | ---------- | ------ | ------ |
| Bazel < 0.19.0 | ![Unknown] | ![No]  | ![No]  |
| Bazel 0.19.x   | ![Yes]     | ![No]  | ![No]  |
| Bazel 0.20.x   | ![Yes]     | ![No]  | ![No]  |
| Bazel 0.21.x   | ![Yes]     | ![No]  | ![No]  |
| Bazel 0.22.x   | ![Yes]     | ![No]  | ![No]  |
| Bazel 0.23.x   | ![Yes]     | ![No]  | ![No]  |
| Bazel 0.24.x   | ![Yes]     | ![No]  | ![No]  |
| Bazel 0.25.x   | ![Yes]     | ![Yes] | ![Yes] |
| Bazel 0.26.x   | ![Yes]     | ![Yes] | ![Yes] |
| Bazel 0.27.x   | ![Yes]     | ![Yes] | ![Yes] |
| Bazel 0.28.0   | ![Yes]     | ![Yes] | ![Yes] |
| Bazel 0.28.1   | ![Unknown] | ![Yes] | ![Yes] |
| Bazel 0.29.0   | ![Unknown] | ![Yes] | ![Yes] |
